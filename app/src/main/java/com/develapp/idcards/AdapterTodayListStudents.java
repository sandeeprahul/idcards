...
public class AdapterTodayListStudents extends RecyclerView.Adapter<AdapterTodayListStudents.MyViewHolder> {
	Context context;
	ArrayList<DataStudents> items;
	int activity;

	public AdapterTodayListStudents(Context context, ArrayList<DataStudents> items, int activity){
	...
	public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewtype){
		View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.item_student
				,parent,false);
	...
	public void onBindViewHolder(final MyViewHolder holder,final int position){
		...
		holder.name_tv_item.setText(items.get(position).student_name);
		...
		holder.edit_tv.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (activity==0){
					((StudentFormActivity)context).dataStudents_.add(items.get(position));
					((StudentFormActivity)context).edit(position,items.get(position));
//					((SHGFormActivity)context).membername_edt.requestFocus();

					//((SHGFormActivity)context).calldelete(items.get(position).id);
				}
				else {
					((IDListActivity)context).editDetailsStudents(position,items.get(position));
	...
	public void showAlert(String msg, final int pos){
		...
		builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
			...
			{
				if (activity==0){
					((StudentFormActivity)context).calldelete(items.get(pos).id);
					removeAt(pos);
				}
				else {
